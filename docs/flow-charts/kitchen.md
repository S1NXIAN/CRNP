...
flowchart TD
    ...
    subgraph Board["Ticket board"]
        N["NOW — oldest first<br/>order code · age timer (red 12 min)<br/>walk-ins enter at settle with their code<br/>NEW: flash ~5 s, badge to age 3 min<br/>all-day = NOW only"]
    ...
    RJ["Cashier rejects a proof"] -.->|"ticket leaves NOW instantly<br/>(board mirrors verified, unserved)"| N
    AP["Cashier approves a rejected proof"] -.->|"rejoins NOW by age —<br/>timer never restarted"| N
