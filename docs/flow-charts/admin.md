...
flowchart TD
    ...
    B --> SA["Staff accounts — create / demote / delete<br/>(name + Google email · Google-only, no passwords)<br/>each evicts the cached role · no public signup"]
    B --> E["Settings"]
    B --> F["Reports"]
    ...
    F --> F1["Sales · reservations · rental bookings<br/>reports: date filters · refunds owed → Mark refunded<br/>· Activity: time · who · action · order code<br/>· per-cashier totals (handledBy)<br/>· scheduled incremental export (by updatedAt)"]
