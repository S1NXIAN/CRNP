...
putenv('FIREBASE_SERVICE_ACCOUNT_JSON');
check('returns null when FIREBASE_SERVICE_ACCOUNT_JSON unset', $refToken->invoke($db) === null);

// 2. Malformed JSON -> null, no crash.
putenv('FIREBASE_SERVICE_ACCOUNT_JSON=not-json{{');
...
putenv('FIREBASE_SERVICE_ACCOUNT_JSON=' . json_encode($sa));
...
putenv('FIREBASE_SERVICE_ACCOUNT_JSON=' . json_encode(['client_email' => $email, 'private_key' => 'whatever']));
